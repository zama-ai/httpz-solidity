...
describe('Rand', function () {
  ...
  it('8 bits', async function () {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      expect(value).to.lessThanOrEqual(0xff);
      values.push(value);
    }
    // Expect at least two different generated values.
    const unique = new Set(values);
    expect(unique.size).to.greaterThanOrEqual(2);
  });
  ...
  it('16 bits', async function () {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      expect(value).to.lessThanOrEqual(0xffff);
      if (value > 0xff) {
        has16bit = true;
      }
      values.push(value);
  ...
  it('32 bits', async function () {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      expect(value).to.lessThanOrEqual(0xffffffff);
      if (value > 0xffff) {
        has32bit = true;
      }
      values.push(value);
