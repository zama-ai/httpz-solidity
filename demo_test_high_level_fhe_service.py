...
def transfer(contract, to, account, amount):
    ...
    res = requests.get("http://40.88.145.138:5000/encrypt?number={}".format(amount))
    json_res = json.loads(res.text)
    input = bytes.fromhex(json_res["encrypted"])
...
def reencrypt(contract, account, cks_file, ct_file, expected):
    ...
    ct = contract.functions.balanceOf().call({"from": account.address})
    print("len(ct) =", len(ct))
    print("retrieving alice's balance took %s seconds" % (time.time() - start))

    ct_hex = binascii.hexlify(ct).decode()
    ct_json = {"encrypted": ct_hex}

    res = requests.post("http://40.88.145.138:5000/decrypt", json=ct_json)
    decrpyted = json.loads(res.text)["decrypted"]

    print("expected: " + str(expected))
    print("extract_number: " + str(decrpyted))
    assert decrpyted == expected
...
w3 = Web3(
    Web3.HTTPProvider("http://40.88.145.138:8545", request_kwargs={"timeout": 600})
)
...
bytecode = compiled_sol["contracts"]["EncryptedERC20.sol"]["EncryptedERC20"]["evm"][
    "bytecode"
]["object"]
abi = json.loads(
    compiled_sol["contracts"]["EncryptedERC20.sol"]["EncryptedERC20"]["metadata"]
)["output"]["abi"]
...
res = requests.get("http://40.88.145.138:5000/encrypt?number={}".format(initial_mint))
json_res = json.loads(res.text)
input = bytes.fromhex(json_res["encrypted"])
...
print("estimating gas and sending mint transaction...")
start = time.time()
gas = contract.functions.mint(input).estimate_gas({"value": 0, "from": account.address})
print("mint gas estimation =", gas)
print("mint gas estimation took %s seconds" % (time.time() - start))

start = time.time()
tx = contract.functions.mint(input).transact({"value": 0, "from": account.address})
transaction_hash = tx.hex()
...
reencrypt(
    contract,
    alice_account,
    "/home/keys/users-fhe-keys/alice_cks.bin",
    "ct_to_decrypt.bin",
    20,
)
